...
class GhostNet(nn.Module):
    def __init__(self, cfgs, num_classes=1000, width=1.0, dropout=0.2):
        ...
        self.aspp1 = nn.Sequential(
            conv_bn_relu(batch_norm, 480, 128, kernel_size=1, padding=0),
            conv_bn_relu(batch_norm, 128, 128, kernel_size=1, padding=0)
        )
        self.aspp2 = nn.Sequential(
            conv_bn_relu(batch_norm, 480, 128, kernel_size=3, padding=pyramid[0], dilation=pyramid[0]),
            conv_bn_relu(batch_norm, 128, 128, kernel_size=1, padding=0)
        )
        self.aspp3 = nn.Sequential(
            conv_bn_relu(batch_norm, 480, 128, kernel_size=3, padding=pyramid[1], dilation=pyramid[1]),
            conv_bn_relu(batch_norm, 128, 128, kernel_size=1, padding=0)
        )
        self.aspp4 = nn.Sequential(
            conv_bn_relu(batch_norm, 480, 128, kernel_size=3, padding=pyramid[2], dilation=pyramid[2]),
            conv_bn_relu(batch_norm, 128, 128, kernel_size=1, padding=0)
        )      

        self.concat_process = nn.Sequential(
            nn.Dropout2d(p=0.5),
            conv_bn_relu(batch_norm, 128*4, 480, kernel_size=1, padding=0),
            nn.Dropout2d(p=0.5),
            nn.Conv2d(480, 480, 1)
        )

        self.cbam = CBAMBlock(channel=480,reduction=8,kernel_size=7)
